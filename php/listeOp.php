    ...
    function Recherche(criter){
        ...
        else
        {
            ...
            if(
                ...
                || tab.rows[i].cells[4].innerHTML.toLowerCase().includes(criter.toLowerCase())
                || tab.rows[i].cells[5].innerHTML.toLowerCase().includes(criter.toLowerCase())
                || tab.rows[i].cells[6].innerHTML.toLowerCase().includes(criter.toLowerCase())
                )
                {
                    tab.rows[i].style.display='table-row';
                }
...
$conn->set_charset("utf8");
    ...
    $nb=mysqli_num_rows($resultat);
        ...
        while ($row=$resultat->fetch_assoc()) {
            ...
            '<td>'.htmlspecialchars($row['Jeux']).'</td>'.
            '<td>'.htmlspecialchars($row['Nature']).'</td>'.
            '<td>'.htmlspecialchars($row['Play']).'</td>'.
